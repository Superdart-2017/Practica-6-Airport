 ...
 */
Airport Vertex_GetNeighborIndex( const Vertex* v )
{
...
void Graph_Print( Graph* g, int depth )
{
   for( int i = 0; i < g->len; ++i )
   {
      ...
      Airport* airport = &vertex->data; 

      printf("<map.key: %d, map.data_idx: %ld>\n", airport->Id, i);
      printf("IATA: %s, Country: %s, City: %s, Name: %s, UTC: %d\n", 
            airport->iata_code, 
            airport->country, 
            airport->city, 
            airport->name, 
      ...
      if( vertex->neighbors )
      {
         ...
         {
            Airport neighbor = List_Cursor_get( vertex->neighbors );
            
            printf(" %s(W:%f) ", g->vertices[ neighbor.Id ].data.iata_code, neighbor.weight);
         }
         printf("\n");
      }
   }
...
bool Graph_IsNeighborOf(Graph* g, int dest, int src){
   ...
   Vertex_Start(src_vertex);

   while (!Vertex_End(src_vertex)){
      Airport neighbor = Vertex_GetNeighborIndex(src_vertex);
      int neighbor_idx = neighbor.Id;

      if (neighbor_idx == dest_idx){
         return true;
      }
      Vertex_Next(src_vertex);
   }
   return false;
}
...
float Graph_GetWeight(Graph* g, int start, int finish){
   ...
   for(Vertex_Start(vertex); !Vertex_End(vertex);Vertex_Next(vertex) ){
      Airport neighbor = Vertex_GetNeighborIndex(vertex);
      
      if(neighbor.Id == finish_idx){
...
int main()
{
   ...
   Airport aeropuerto1 = {100, "MEX", "MEXICO", "MEXICO CITY", "AEROPUERTO INTERNACIONAL BENITO JUÁREZ", -6};
   Airport aeropuerto2 = {120, "LHR", "UNITED KINGDOM", "LONDON", "LONDON HEATHROW", 0};
   Airport aeropuerto3 = {130, "MAD", "SPAIN", "MADRID", "MADRID-BARAJAS", 1};
   Airport aeropuerto4 = {140, "FRA", "GERMANY", "FRANKFURT", "FLUGHAFEN FRANKFURT AM MAIN", 1};
   Airport aeropuerto5 = {150, "CDG", "FRANCE", "PARIS", "CHARLES DE GAULLE", 1};
   ...
   Graph_AddWeightedEdge(grafo, 100, 120, 8900.0f);  
   Graph_AddWeightedEdge(grafo, 120, 130, 1260.0f); 
   Graph_AddWeightedEdge(grafo, 130, 140, 1370.0f); 
   Graph_AddWeightedEdge(grafo, 140, 150, 475.0f);  
   Graph_AddWeightedEdge(grafo, 150, 100, 1050.0f); 



   // Probar funciones del grafo
   printf("Tamaño del grafo: %d\n", Graph_GetSize(grafo));
   printf("Número de vértices en el grafo: %d\n", Graph_GetLen(grafo));

   Graph_Print( grafo, 1 );
   // imprime el grafo completo (esta versión no usa al segundo argumento)
   for (int i = 0; i < Graph_GetLen(grafo); i++) {
   Item vertex_val = Graph_GetDataByIndex(grafo, i);
   printf("***[%d]%d => ", i, vertex_val.Id);

   Vertex* vertex = Graph_GetVertexByIndex(grafo, i);

   Vertex_Start(vertex);
   while (!Vertex_End(vertex)) {
      Airport neighbor = Vertex_GetNeighborIndex(vertex);
      Item neighbor_data = Graph_GetDataByIndex(grafo, neighbor.Id); 

      printf("%d(W:%f) ", neighbor_data.Id, neighbor.weight); 
      Vertex_Next(vertex);
   }
   printf("\n");
}
